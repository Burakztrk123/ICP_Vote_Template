...
thread_local! {
 ...
 fn get_proposal(key: u64) -> Option<Proposal> {
   Proposal_MAP.with(|p: &RefCell<StableBTreeMap<u64, Proposal,Memory>>| p.borrow().get(&key)) 
 }

 #[ic_cdk::query]
 fn get_proposal_count() -> u64 {
    Proposal_MAP.with(|p: &RefCell<StableBTreeMap<u64, Proposal,Memory>>| p.borrow().len())
 }
 ...
 fn create_proposal(key: u64, proposal: CreateProposal) -> Option<Proposal> {
    ...
    Proposal_MAP.with(|p: &RefCell<StableBTreeMap<u64, Proposal,Memory<std::rc::Rc<RefCell<Vec<u8>>>>>>| p.borrow_mut().insert(key, value))
 }
 ...
 fn vote(key: u64, choice: Choice) -> Result<(), VoteError> {
    Proposal_MAP.with(|p: &RefCell<StableBTreeMap<u64, Proposal,Memory>>| {
        ...
        let res: RefMut<StableBTreeMap<u64, Proposal,Memory>>=p.borrow_mut().insert(key, proposal);
